...
namespace eBooks.ServiceLayer.Contracts.Identity
{
    public interface IUserManagerService : IDisposable
    {
        ...
        public Task<bool> IsExistUserNameAsync(string? userName, Guid? id);

        public Task<bool> IsExistIdAsync(Guid id);

        public Task<bool> IsUserNameActivatedAsync(string userName);

        public Task<bool> IsExistUserHaveRole(string userName);

        public Task<string> UserRegisterAsync(RegisterViewModel viewModel);

        public Task<bool> UserIsLockedOutAsync(string phoneNumber);

        public Task<bool> ActivationPhoneNumberAsync(string phoneNumber);

        public Task ClearAccessFailedCountAsync(string phoneNumber);

        public Task ChangeSecurityStampAsync(string phoneNumber);

        public Task<bool> ModifyPhoneNumber(string oldPhoneNumber, string newPhoneNumber);

        public Task<string> CreatePasswordResetTokenAsync(string phoneNumber);

        public Task<bool> ModifyPasswordAsync(string phoneNumber, string token, string password);

        #endregion Account Part

        #region User

        public Task<bool> IsExistPasswordAsync(string userName, string password);

        public Task<ListViewModel> GetAllUserAsync(ListViewModel viewModel, bool isDelete);

        public Task<UserViewModel> AddUserAsync(AddUserViewModel viewModel);

        public Task<EditUserViewModel> GetUserForEditAsync(string userName);

        public Task<UserViewModel> EditUserAsync(EditUserViewModel viewModel);

        public Task<IdentityResult> ChangeUserPasswordAsync(ChangePasswordViewModel viewModel);

        public Task<IdentityResult> DeleteUserAsync(Guid id);

        public Task<IdentityResult> ReactivateUserAsync(Guid id);
